...
namespace Boundless {
    ...
    class Octree {
        public:
            ...
            void visitAllConditional(OctreeNode& node, std::function< bool(uint64_t nodeLocationalCode, OctreeNode& node) > lambda);
            void visitAllBottomUp(OctreeNode& node, std::function< void(uint64_t nodeLocationalCode, OctreeNode& node) > lambda);
            void visitAllBy3DDistance(OctreeNode& node, uint16_t maxDistance, std::function< void(uint64_t nodeLocationalCode, OctreeNode& node) > lambda);
